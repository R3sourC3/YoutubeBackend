...
if (!avatar.url) {
  throw new ApiError(400,"Error while uploaing on avatar")
...
const user=await User.findByIdAndUpdate(
  ...
  return res.status(200)
  .json(new ApiResponse(200,user,"avatar updated suucessfully"))
})
...
if (!coverImage.url) {
  throw new ApiError(400,"Error while uploaing on cover Image")
...
const user=await User.findByIdAndUpdate(
  ...
  return res.status(200)
  .json(new ApiResponse(200,user,"coverImage updated suucessfully"))
})
